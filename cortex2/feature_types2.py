...
LAMP2.connect(os.getenv('LAMP_ACCESS_KEY'), os.getenv('LAMP_SECRET_KEY'),
             os.getenv('LAMP_SERVER_ADDRESS', 'api.lamp.digital'), archive=True)
...
def raw_feature(name, dependencies):
    ...
    def _wrapper1(func):
        ...
        def _get_sensor_feature(name, **kwgs):
            ...
            """
            data_next = []
            data = LAMP2.SensorEvent.all_by_participant(kwgs['id'],
                                               origin=name,
            ...
            while (kwgs['recursive'] and
                ...
                to = data[-1]['timestamp']
                data_next = LAMP2.SensorEvent.all_by_participant(kwgs['id'],
                                                        origin=name,
                                                        _from=int(kwgs['start']),
                                                        to=int(to),
                                                        _limit=int(kwgs['_limit']))['data']
            ...
            if name == "lamp.device_state":
                data_next = []
                data_screen_state = LAMP2.SensorEvent.all_by_participant(kwgs['id'],
        ...
        def _get_game_feature(name, **kwgs):
            ...
            activity_ids = [activity['id'] for activity in
                    LAMP2.Activity.all_by_participant(kwgs['id'])['data']
                    if activity['spec'] == name]

            data = LAMP2.ActivityEvent.all_by_participant(kwgs['id'],
                                               _from=int(kwgs['start']),
                                               to=int(kwgs['end']),
                                               _limit=int(kwgs['_limit']))['data']
            while (kwgs['recursive'] and
                ...
                to = data[-1]['timestamp']
                data_next = LAMP2.ActivityEvent.all_by_participant(kwgs['id'],
                                                        _from=int(kwgs['start']),
                                                        to=int(to),
                                                        _limit=int(kwgs['_limit']))['data']
                data += data_next
    ...
    def _wrapper1(func):
        def _wrapper2(*args, **kwargs):
            ...
            def _primary_attach(func, name, *args, **kwargs):
                ...
                """
                try:
                    attachments = LAMP2.Type.get_attachment(kwargs['id'], name)['data']
                ...
                LAMP2.Type.set_attachment(kwargs['id'], 'me', attachment_key=name, body=_body_new)
...
def delete_attach(participant, features=None):
    ...
    """
    attachments= LAMP2.Type.list_attachments(participant)['data']
    if features is None: features=attachments
    for feature in attachments:
        if feature.startswith('cortex'):
            if feature in features:
                LAMP2.Type.set_attachment(participant, 'me', attachment_key=feature, body=None)
